...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    record = ExitInterviewRecord(normalize_payload(payload))
    record.validate()
    return record.to_dict()

def create(payload: dict[str, Any]) -> ExitInterviewRecord:
    record = ExitInterviewRecord(normalize_payload(payload))
    record.validate()
    return record
