...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    record = KnowledgeHandoffRecord(normalize_payload(payload))
    record.validate()
    return record.to_dict()

def create(payload: dict[str, Any]) -> KnowledgeHandoffRecord:
    record = KnowledgeHandoffRecord(normalize_payload(payload))
    record.validate()
    return record
