...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    record = FirstWeekPlanRecord(normalize_payload(payload))
    record.validate()
    return record.to_dict()

def create(payload: dict[str, Any]) -> FirstWeekPlanRecord:
    record = FirstWeekPlanRecord(normalize_payload(payload))
    record.validate()
    return record
