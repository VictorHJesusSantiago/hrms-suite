...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    record = BuddyAssignmentRecord(normalize_payload(payload))
    record.validate()
    return record.to_dict()

def create(payload: dict[str, Any]) -> BuddyAssignmentRecord:
    record = BuddyAssignmentRecord(normalize_payload(payload))
    record.validate()
    return record
