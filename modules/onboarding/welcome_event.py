...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    record = WelcomeEventRecord(normalize_payload(payload))
    record.validate()
    return record.to_dict()

def create(payload: dict[str, Any]) -> WelcomeEventRecord:
    record = WelcomeEventRecord(normalize_payload(payload))
    record.validate()
    return record
