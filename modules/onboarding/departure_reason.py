...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    record = DepartureReasonRecord(normalize_payload(payload))
    record.validate()
    return record.to_dict()

def create(payload: dict[str, Any]) -> DepartureReasonRecord:
    record = DepartureReasonRecord(normalize_payload(payload))
    record.validate()
    return record
