...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    record = PulseCheckRecord(normalize_payload(payload))
    record.validate()
    return record.to_dict()

def create(payload: dict[str, Any]) -> PulseCheckRecord:
    record = PulseCheckRecord(normalize_payload(payload))
    record.validate()
    return record
