...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    record = CultureSignalRecord(normalize_payload(payload))
    record.validate()
    return record.to_dict()

def create(payload: dict[str, Any]) -> CultureSignalRecord:
    record = CultureSignalRecord(normalize_payload(payload))
    record.validate()
    return record
