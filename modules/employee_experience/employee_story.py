...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    record = EmployeeStoryRecord(normalize_payload(payload))
    record.validate()
    return record.to_dict()

def create(payload: dict[str, Any]) -> EmployeeStoryRecord:
    record = EmployeeStoryRecord(normalize_payload(payload))
    record.validate()
    return record
