"""Recurso de experiencia: Internal Communication."""
from dataclasses import dataclass, field
from typing import Any

from domain.shared import normalize_payload, require_fields, utc_now

RESOURCE = "Internal Communication"
FIELDS = ('employee_id', 'category', 'score')
DEFAULT_STATUS = "active"

@dataclass
class InternalCommunicationRecord:
    data: dict[str, Any]
...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    record = InternalCommunicationRecord(normalize_payload(payload))
    record.validate()
    return record.to_dict()

def create(payload: dict[str, Any]) -> InternalCommunicationRecord:
    record = InternalCommunicationRecord(normalize_payload(payload))
    record.validate()
    return record
