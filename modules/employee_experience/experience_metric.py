...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    record = ExperienceMetricRecord(normalize_payload(payload))
    record.validate()
    return record.to_dict()

def create(payload: dict[str, Any]) -> ExperienceMetricRecord:
    record = ExperienceMetricRecord(normalize_payload(payload))
    record.validate()
    return record
