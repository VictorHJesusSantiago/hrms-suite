...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    record = ExceptionCaseRecord(normalize_payload(payload))
    record.validate()
    return record.to_dict()

def create(payload: dict[str, Any]) -> ExceptionCaseRecord:
    record = ExceptionCaseRecord(normalize_payload(payload))
    record.validate()
    return record
