...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    record = WellbeingCheckRecord(normalize_payload(payload))
    record.validate()
    return record.to_dict()

def create(payload: dict[str, Any]) -> WellbeingCheckRecord:
    record = WellbeingCheckRecord(normalize_payload(payload))
    record.validate()
    return record
