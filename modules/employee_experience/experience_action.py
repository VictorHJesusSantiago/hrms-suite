...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    record = ExperienceActionRecord(normalize_payload(payload))
    record.validate()
    return record.to_dict()

def create(payload: dict[str, Any]) -> ExperienceActionRecord:
    record = ExperienceActionRecord(normalize_payload(payload))
    record.validate()
    return record
