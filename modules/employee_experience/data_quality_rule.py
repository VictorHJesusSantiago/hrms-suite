...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    record = DataQualityRuleRecord(normalize_payload(payload))
    record.validate()
    return record.to_dict()

def create(payload: dict[str, Any]) -> DataQualityRuleRecord:
    record = DataQualityRuleRecord(normalize_payload(payload))
    record.validate()
    return record
