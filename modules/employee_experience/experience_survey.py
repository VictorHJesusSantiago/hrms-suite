...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    record = ExperienceSurveyRecord(normalize_payload(payload))
    record.validate()
    return record.to_dict()

def create(payload: dict[str, Any]) -> ExperienceSurveyRecord:
    record = ExperienceSurveyRecord(normalize_payload(payload))
    record.validate()
    return record
