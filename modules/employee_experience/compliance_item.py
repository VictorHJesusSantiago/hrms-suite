...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    record = ComplianceItemRecord(normalize_payload(payload))
    record.validate()
    return record.to_dict()

def create(payload: dict[str, Any]) -> ComplianceItemRecord:
    record = ComplianceItemRecord(normalize_payload(payload))
    record.validate()
    return record
