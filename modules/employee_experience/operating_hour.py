...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    record = OperatingHourRecord(normalize_payload(payload))
    record.validate()
    return record.to_dict()

def create(payload: dict[str, Any]) -> OperatingHourRecord:
    record = OperatingHourRecord(normalize_payload(payload))
    record.validate()
    return record
