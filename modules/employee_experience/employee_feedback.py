...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    record = EmployeeFeedbackRecord(normalize_payload(payload))
    record.validate()
    return record.to_dict()

def create(payload: dict[str, Any]) -> EmployeeFeedbackRecord:
    record = EmployeeFeedbackRecord(normalize_payload(payload))
    record.validate()
    return record
