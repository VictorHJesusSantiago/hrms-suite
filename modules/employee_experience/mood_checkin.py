...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    record = MoodCheckinRecord(normalize_payload(payload))
    record.validate()
    return record.to_dict()

def create(payload: dict[str, Any]) -> MoodCheckinRecord:
    record = MoodCheckinRecord(normalize_payload(payload))
    record.validate()
    return record
