...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    record = ListeningCampaignRecord(normalize_payload(payload))
    record.validate()
    return record.to_dict()

def create(payload: dict[str, Any]) -> ListeningCampaignRecord:
    record = ListeningCampaignRecord(normalize_payload(payload))
    record.validate()
    return record
