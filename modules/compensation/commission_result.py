...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    record = CommissionResultRecord(normalize_payload(payload))
    record.validate()
    return record.to_dict()

def create(payload: dict[str, Any]) -> CommissionResultRecord:
    record = CommissionResultRecord(normalize_payload(payload))
    record.validate()
    return record
