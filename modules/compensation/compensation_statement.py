...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    record = CompensationStatementRecord(normalize_payload(payload))
    record.validate()
    return record.to_dict()

def create(payload: dict[str, Any]) -> CompensationStatementRecord:
    record = CompensationStatementRecord(normalize_payload(payload))
    record.validate()
    return record
