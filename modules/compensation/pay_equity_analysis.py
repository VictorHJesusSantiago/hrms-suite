...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    record = PayEquityAnalysisRecord(normalize_payload(payload))
    record.validate()
    return record.to_dict()

def create(payload: dict[str, Any]) -> PayEquityAnalysisRecord:
    record = PayEquityAnalysisRecord(normalize_payload(payload))
    record.validate()
    return record
