...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    record = PromotionCaseRecord(normalize_payload(payload))
    record.validate()
    return record.to_dict()

def create(payload: dict[str, Any]) -> PromotionCaseRecord:
    record = PromotionCaseRecord(normalize_payload(payload))
    record.validate()
    return record
