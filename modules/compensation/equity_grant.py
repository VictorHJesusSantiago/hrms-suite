...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    record = EquityGrantRecord(normalize_payload(payload))
    record.validate()
    return record.to_dict()

def create(payload: dict[str, Any]) -> EquityGrantRecord:
    record = EquityGrantRecord(normalize_payload(payload))
    record.validate()
    return record
