...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    record = CompensationMetricRecord(normalize_payload(payload))
    record.validate()
    return record.to_dict()

def create(payload: dict[str, Any]) -> CompensationMetricRecord:
    record = CompensationMetricRecord(normalize_payload(payload))
    record.validate()
    return record
