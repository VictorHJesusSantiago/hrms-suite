...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    record = SalaryReviewRecord(normalize_payload(payload))
    record.validate()
    return record.to_dict()

def create(payload: dict[str, Any]) -> SalaryReviewRecord:
    record = SalaryReviewRecord(normalize_payload(payload))
    record.validate()
    return record
