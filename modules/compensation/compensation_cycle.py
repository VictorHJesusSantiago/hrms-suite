...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    record = CompensationCycleRecord(normalize_payload(payload))
    record.validate()
    return record.to_dict()

def create(payload: dict[str, Any]) -> CompensationCycleRecord:
    record = CompensationCycleRecord(normalize_payload(payload))
    record.validate()
    return record
