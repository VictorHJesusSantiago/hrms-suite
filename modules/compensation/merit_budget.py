...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    record = MeritBudgetRecord(normalize_payload(payload))
    record.validate()
    return record.to_dict()

def create(payload: dict[str, Any]) -> MeritBudgetRecord:
    record = MeritBudgetRecord(normalize_payload(payload))
    record.validate()
    return record
