...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    record = RewardPreferenceRecord(normalize_payload(payload))
    record.validate()
    return record.to_dict()

def create(payload: dict[str, Any]) -> RewardPreferenceRecord:
    record = RewardPreferenceRecord(normalize_payload(payload))
    record.validate()
    return record
