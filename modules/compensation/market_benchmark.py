...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    record = MarketBenchmarkRecord(normalize_payload(payload))
    record.validate()
    return record.to_dict()

def create(payload: dict[str, Any]) -> MarketBenchmarkRecord:
    record = MarketBenchmarkRecord(normalize_payload(payload))
    record.validate()
    return record
