...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    record = BenefitUsageRecord(normalize_payload(payload))
    record.validate()
    return record.to_dict()

def create(payload: dict[str, Any]) -> BenefitUsageRecord:
    record = BenefitUsageRecord(normalize_payload(payload))
    record.validate()
    return record
