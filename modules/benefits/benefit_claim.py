...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    record = BenefitClaimRecord(normalize_payload(payload))
    record.validate()
    return record.to_dict()

def create(payload: dict[str, Any]) -> BenefitClaimRecord:
    record = BenefitClaimRecord(normalize_payload(payload))
    record.validate()
    return record
