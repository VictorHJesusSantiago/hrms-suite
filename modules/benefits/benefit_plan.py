...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    record = BenefitPlanRecord(normalize_payload(payload))
    record.validate()
    return record.to_dict()

def create(payload: dict[str, Any]) -> BenefitPlanRecord:
    record = BenefitPlanRecord(normalize_payload(payload))
    record.validate()
    return record
