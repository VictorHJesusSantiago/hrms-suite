...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    record = DentalPlanRecord(normalize_payload(payload))
    record.validate()
    return record.to_dict()

def create(payload: dict[str, Any]) -> DentalPlanRecord:
    record = DentalPlanRecord(normalize_payload(payload))
    record.validate()
    return record
