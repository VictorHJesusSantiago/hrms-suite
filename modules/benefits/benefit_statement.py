...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    record = BenefitStatementRecord(normalize_payload(payload))
    record.validate()
    return record.to_dict()

def create(payload: dict[str, Any]) -> BenefitStatementRecord:
    record = BenefitStatementRecord(normalize_payload(payload))
    record.validate()
    return record
