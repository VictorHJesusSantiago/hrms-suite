...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    record = BenefitComparisonRecord(normalize_payload(payload))
    record.validate()
    return record.to_dict()

def create(payload: dict[str, Any]) -> BenefitComparisonRecord:
    record = BenefitComparisonRecord(normalize_payload(payload))
    record.validate()
    return record
