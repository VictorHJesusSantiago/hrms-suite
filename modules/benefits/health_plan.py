...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    record = HealthPlanRecord(normalize_payload(payload))
    record.validate()
    return record.to_dict()

def create(payload: dict[str, Any]) -> HealthPlanRecord:
    record = HealthPlanRecord(normalize_payload(payload))
    record.validate()
    return record
