...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    record = TransportVoucherRecord(normalize_payload(payload))
    record.validate()
    return record.to_dict()

def create(payload: dict[str, Any]) -> TransportVoucherRecord:
    record = TransportVoucherRecord(normalize_payload(payload))
    record.validate()
    return record
