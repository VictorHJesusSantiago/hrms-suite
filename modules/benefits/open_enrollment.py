...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    record = OpenEnrollmentRecord(normalize_payload(payload))
    record.validate()
    return record.to_dict()

def create(payload: dict[str, Any]) -> OpenEnrollmentRecord:
    record = OpenEnrollmentRecord(normalize_payload(payload))
    record.validate()
    return record
