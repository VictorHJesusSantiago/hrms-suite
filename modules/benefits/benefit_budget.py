...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    record = BenefitBudgetRecord(normalize_payload(payload))
    record.validate()
    return record.to_dict()

def create(payload: dict[str, Any]) -> BenefitBudgetRecord:
    record = BenefitBudgetRecord(normalize_payload(payload))
    record.validate()
    return record
