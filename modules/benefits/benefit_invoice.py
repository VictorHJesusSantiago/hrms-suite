...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    record = BenefitInvoiceRecord(normalize_payload(payload))
    record.validate()
    return record.to_dict()

def create(payload: dict[str, Any]) -> BenefitInvoiceRecord:
    record = BenefitInvoiceRecord(normalize_payload(payload))
    record.validate()
    return record
