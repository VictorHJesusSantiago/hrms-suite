"""Recurso de beneficios: Eligibility Snapshot."""
from dataclasses import dataclass, field
from typing import Any

from domain.shared import normalize_payload, require_fields, utc_now

RESOURCE = "Eligibility Snapshot"
FIELDS = ('employee_id', 'provider', 'value')
DEFAULT_STATUS = "active"

@dataclass
class EligibilitySnapshotRecord:
    data: dict[str, Any]
...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    record = EligibilitySnapshotRecord(normalize_payload(payload))
    record.validate()
    return record.to_dict()

def create(payload: dict[str, Any]) -> EligibilitySnapshotRecord:
    record = EligibilitySnapshotRecord(normalize_payload(payload))
    record.validate()
    return record
