...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    record = LearningBudgetRecord(normalize_payload(payload))
    record.validate()
    return record.to_dict()

def create(payload: dict[str, Any]) -> LearningBudgetRecord:
    record = LearningBudgetRecord(normalize_payload(payload))
    record.validate()
    return record
