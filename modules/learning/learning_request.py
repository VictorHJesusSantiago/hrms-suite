...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    record = LearningRequestRecord(normalize_payload(payload))
    record.validate()
    return record.to_dict()

def create(payload: dict[str, Any]) -> LearningRequestRecord:
    record = LearningRequestRecord(normalize_payload(payload))
    record.validate()
    return record
