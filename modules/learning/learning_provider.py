...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    record = LearningProviderRecord(normalize_payload(payload))
    record.validate()
    return record.to_dict()

def create(payload: dict[str, Any]) -> LearningProviderRecord:
    record = LearningProviderRecord(normalize_payload(payload))
    record.validate()
    return record
