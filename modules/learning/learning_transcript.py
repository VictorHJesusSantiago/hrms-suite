...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    record = LearningTranscriptRecord(normalize_payload(payload))
    record.validate()
    return record.to_dict()

def create(payload: dict[str, Any]) -> LearningTranscriptRecord:
    record = LearningTranscriptRecord(normalize_payload(payload))
    record.validate()
    return record
