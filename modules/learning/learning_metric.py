...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    record = LearningMetricRecord(normalize_payload(payload))
    record.validate()
    return record.to_dict()

def create(payload: dict[str, Any]) -> LearningMetricRecord:
    record = LearningMetricRecord(normalize_payload(payload))
    record.validate()
    return record
