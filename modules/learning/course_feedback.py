...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    record = CourseFeedbackRecord(normalize_payload(payload))
    record.validate()
    return record.to_dict()

def create(payload: dict[str, Any]) -> CourseFeedbackRecord:
    record = CourseFeedbackRecord(normalize_payload(payload))
    record.validate()
    return record
