...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    record = SkillBadgeRecord(normalize_payload(payload))
    record.validate()
    return record.to_dict()

def create(payload: dict[str, Any]) -> SkillBadgeRecord:
    record = SkillBadgeRecord(normalize_payload(payload))
    record.validate()
    return record
