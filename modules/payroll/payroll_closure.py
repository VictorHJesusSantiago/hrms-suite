...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    record = PayrollClosureRecord(normalize_payload(payload))
    record.validate()
    return record.to_dict()

def create(payload: dict[str, Any]) -> PayrollClosureRecord:
    record = PayrollClosureRecord(normalize_payload(payload))
    record.validate()
    return record
