...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    record = PayrollValidationRecord(normalize_payload(payload))
    record.validate()
    return record.to_dict()

def create(payload: dict[str, Any]) -> PayrollValidationRecord:
    record = PayrollValidationRecord(normalize_payload(payload))
    record.validate()
    return record
