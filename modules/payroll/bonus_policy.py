...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    record = BonusPolicyRecord(normalize_payload(payload))
    record.validate()
    return record.to_dict()

def create(payload: dict[str, Any]) -> BonusPolicyRecord:
    record = BonusPolicyRecord(normalize_payload(payload))
    record.validate()
    return record
