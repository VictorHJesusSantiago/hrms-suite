...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    record = PayrollItemRecord(normalize_payload(payload))
    record.validate()
    return record.to_dict()

def create(payload: dict[str, Any]) -> PayrollItemRecord:
    record = PayrollItemRecord(normalize_payload(payload))
    record.validate()
    return record
