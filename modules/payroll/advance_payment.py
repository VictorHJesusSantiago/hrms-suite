...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    record = AdvancePaymentRecord(normalize_payload(payload))
    record.validate()
    return record.to_dict()

def create(payload: dict[str, Any]) -> AdvancePaymentRecord:
    record = AdvancePaymentRecord(normalize_payload(payload))
    record.validate()
    return record
